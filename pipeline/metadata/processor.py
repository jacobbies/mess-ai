...
class MetadataProcessor:
    ...
    def __init__(self, data_dir: Optional[str] = None, metadata_dir: Optional[str] = None):
        if data_dir is None:
    ...
    def parse_smd_filename(self, filename: str) -> Dict[str, str]:
        ...
        if len(parts) > 3:
            ...
            try:
                ...
                result["recording_date"] = datetime(year, month, day).date().isoformat()
            except:
                pass
    ...
    def enhance_metadata(self, parsed: Dict[str, str]) -> TrackMetadata:
        ...
        return TrackMetadata(
            ...
            performer_name=None,  # SMD doesn't provide performer names
            instrument="Piano",  # SMD is piano-only dataset
            recording_date=datetime.fromisoformat(parsed["recording_date"]).date() if isinstance(parsed.get("recording_date"), str) else None,
            year_composed=None,  # Would need additional data source
            dataset_source="SMD",
            tags=tags,
            # New optional fields from the model
            movement_name=None,
            tempo_marking=None,
            ensemble=None,
    ...
    def load_metadata_dict(self) -> Dict[str, TrackMetadata]:
        ...
        for _, row in df.iterrows():
            ...
            required_strings = ['track_id', 'filename', 'title', 'composer', 'composer_full']
            for field in required_strings:
                value = track_data.get(field)
                if pd.isna(value) or value is None:
                    cleaned_data[field] = "Unknown"  # Provide default for required fields
                else:
                    cleaned_data[field] = str(value)
            
            # Optional string fields
            optional_strings = ['opus', 'movement', 'movement_name', 'form', 'key_signature', 
                              'tempo_marking', 'performer_id', 'performer_name', 'ensemble', 
                              'dataset_version']
            ...
            value = track_data.get('instrument')
            cleaned_data['instrument'] = str(value) if value and not pd.isna(value) else "Piano"
            
            # Dataset source with default
            value = track_data.get('dataset_source')
            cleaned_data['dataset_source'] = str(value) if value and not pd.isna(value) else "SMD"
            
            # Era field (Literal type)
            era_value = track_data.get('era')
            if era_value and not pd.isna(era_value) and era_value in ["Medieval", "Renaissance", "Baroque", "Classical", "Romantic", "Modern", "Contemporary"]:
                cleaned_data['era'] = era_value
            else:
                cleaned_data['era'] = None
            
            # Difficulty level (Literal type)
            difficulty_value = track_data.get('difficulty_level')
            if difficulty_value and not pd.isna(difficulty_value) and difficulty_value in ["Beginner", "Intermediate", "Advanced", "Professional"]:
                cleaned_data['difficulty_level'] = difficulty_value
            else:
                cleaned_data['difficulty_level'] = None
            
            # Recording date
            date_value = track_data.get('recording_date')
            if date_value and not pd.isna(date_value):
                try:
                    if isinstance(date_value, str):
                        cleaned_data['recording_date'] = datetime.fromisoformat(date_value).date()
                    else:
                        cleaned_data['recording_date'] = None
                except:
                    cleaned_data['recording_date'] = None
            else:
                cleaned_data['recording_date'] = None
            ...
            # Integer fields
            int_fields = ['year_composed', 'popular_rank']
            ...
            # Float fields
            value = track_data.get('duration_seconds')
            if value and not pd.isna(value):
                try:
                    cleaned_data['duration_seconds'] = float(value)
                except:
            ...
            # Tags field (list)
            tags_value = track_data.get('tags')
            if isinstance(tags_value, list):
                cleaned_data['tags'] = tags_value
            elif tags_value and not pd.isna(tags_value):
                if isinstance(tags_value, str):
                    cleaned_data['tags'] = [t.strip() for t in tags_value.split(',') if t.strip()]
                else:
                    cleaned_data['tags'] = []
            else:
                cleaned_data['tags'] = []
            ...
            try:
                metadata = TrackMetadata(**cleaned_data)
                metadata_dict[metadata.track_id] = metadata
            except Exception as e:
                print(f"Error creating metadata for {cleaned_data.get('track_id', 'unknown')}: {e}")
                continue
